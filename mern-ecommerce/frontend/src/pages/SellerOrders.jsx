  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {!error && orders.length === 0 && (
        <EmptyState title="No orders yet" subtitle="Orders from customers will appear here" />
      )}

      <div className="space-y-4">
        {orders.map((order) => (
          <div key={order._id} className="card p-4 flex flex-col sm:flex-row sm:items-center gap-4">
            <img
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 mt-1">
                {order.customer?.name} · {order.customerDetails?.city}, {order.customerDetails?.state}
              </p>
